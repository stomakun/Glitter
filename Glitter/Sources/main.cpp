...
static const char *fragment_shader_text = "#version 330 core\n"
    ...
    "    vec4 b = texelFetch(B, ivec2(col, i * 4), 0);\n"
    "    vec4 c = texelFetch(B, ivec2(col, i * 4 + 1), 0);\n"
    "    vec4 d = texelFetch(B, ivec2(col, i * 4 + 2), 0);\n"
    "    vec4 e = texelFetch(B, ivec2(col, i * 4 + 3), 0);\n"
    "    x += dot(a, vec4(b.x, c.x, d.x, e.x))\n;"
    "    y += dot(a, vec4(b.y, c.y, d.y, e.y))\n;"
    "    z += dot(a, vec4(b.z, c.z, d.z, e.z))\n;"
    "    w += dot(a, vec4(b.w, c.w, d.w, e.w))\n;"
    "  }\n"
    "  color = vec4(x, y, z, w);"
    "}\n";
